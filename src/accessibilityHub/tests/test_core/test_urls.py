...
class TestURLS(TestCase):
    ...
    def test_login(self):
        ...
        response = self.client.post(loginUrl, {'username': 'DoesNotExist', 'password': 'Jones.2020'})
        self.assertTemplateUsed(response, 'core/login.html')
        self.assertTrue(response.context['errors'])  # contains errors in it.

        response = self.client.post(loginUrl, {'username': 'james', 'password': 'InvalidPassword'})
        self.assertTemplateUsed(response, 'core/login.html')
