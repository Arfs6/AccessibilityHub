...
class TestURLS(TestCase):
    ...
    fixtures: typing.ClassVar = ['User.json', 'Tool.json', 'Owner.json', 'Review.json']
    ...
    def test_owner(self):
        ...
        response = self.client.get(reverse(ownerUrl, kwargs={'base36Id': owner.base36Id}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'reviews/owner.html')
        self.assertEqual(response.context['owner'], owner)
        self.assertQuerySetEqual(response.context['tools'], owner.tools.filter(verified=True).all())
    ...
    def test_tool(self):
        ...
        tool = models.Tool.allVerified().first()
        user = User.objects.exclude(reviews__tool=tool).first()  # user hasn't reviewed tool
